<template>
  <div class="p-6 max-w-[1400px] mx-auto">
    <PageHeader title="Commandes fournisseurs" :subtitle="`${filtered.length} commande(s)`">
      <template #actions>
    ...
    <div class="card-premium rounded-2xl p-4 mb-6 flex flex-wrap gap-3 items-center">
      <div class="relative flex-1 min-w-[200px]">
        ...
        <input v-model="search" type="search" placeholder="N° commande, fournisseur…" class="input-field pl-10" />
      </div>
      <select v-model="filterFournisseur" class="select-field w-auto min-w-[170px]">
        <option value="">Tous les fournisseurs</option>
        <option v-for="f in fournisseurs" :key="f">{{ f }}</option>
      </select>
...
const fournisseurs = computed(() => [...new Set(PURCHASE_ORDERS.map(o => o.fournisseurNom))])
const filtered = computed(() => PURCHASE_ORDERS.filter(o => {
  if (search.value && !`${o.numero} ${o.fournisseurNom}`.toLowerCase().includes(search.value.toLowerCase())) return false
  if (filterFournisseur.value && o.fournisseurNom !== filterFournisseur.value) return false
  if (filterStatut.value && o.statut !== filterStatut.value) return false
  return true
}))

const kpis = computed(() => [
  { label: 'Brouillon', count: PURCHASE_ORDERS.filter(o => o.statut === 'BROUILLON').length, color: 'text-muted-foreground' },
  { label: 'Envoyées', count: PURCHASE_ORDERS.filter(o => o.statut === 'ENVOYEE').length, color: 'text-blue-600' },
  { label: 'En retard', count: PURCHASE_ORDERS.filter(o => isLate(o)).length, color: 'text-red-500' },
  { label: 'Livrées', count: PURCHASE_ORDERS.filter(o => o.statut === 'LIVREE').length, color: 'text-emerald-600' },
])

function isLate(o: typeof PURCHASE_ORDERS[0]) {
