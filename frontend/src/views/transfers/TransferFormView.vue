<template>
  <div class="p-6 max-w-[900px] mx-auto">
    <PageHeader title="Nouveau transfert" subtitle="Mouvement de stock entre entrepôts" back="Transferts" />
    <form @submit.prevent="submit" class="space-y-6">
      <div class="card-premium rounded-2xl p-6">
        <h3 class="font-semibold text-foreground mb-4">Entrepôts</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div><label class="label-field">Entrepôt source *</label>
            <select v-model="form.sourceId" class="select-field" required>
          ...
          <div><label class="label-field">Entrepôt destination *</label>
            <select v-model="form.destId" class="select-field" required>
              <option :value="null" disabled>— Sélectionner —</option>
              <option v-for="e in warehouses.filter(w => w.id !== form.sourceId)" :key="e.id" :value="e.id">{{ e.code }} — {{ e.nom }}</option>
            </select>
          </div>
          <div class="md:col-span-2"><label class="label-field">Note</label><input v-model="form.note" type="text" class="input-field" /></div>
        </div>
      </div>
      <div class="card-premium rounded-2xl overflow-hidden">
        <div class="px-6 py-4 border-b border-border flex items-center justify-between">
          <h3 class="font-semibold">Produits à transférer</h3>
          <button type="button" @click="addLine" class="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold bg-emerald-50 border border-emerald-200 text-emerald-700 hover:bg-emerald-100 transition-colors"><Plus :size="13" /> Ajouter</button>
        </div>
      ...
      <div class="flex items-center gap-3 justify-end">
        <RouterLink to="/transfers" class="px-4 py-2.5 rounded-xl border border-border text-sm font-medium hover:bg-muted transition-colors">Annuler</RouterLink>
        <button type="submit" class="btn-primary px-5 py-2.5 rounded-xl text-sm font-semibold text-white">Créer le transfert</button>
      </div>
    </form>
  </div>
</template>
