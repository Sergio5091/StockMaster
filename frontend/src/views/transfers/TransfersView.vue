<template>
  <div class="p-6 max-w-[1400px] mx-auto">
    <PageHeader title="Transferts" :subtitle="`${total} transfert(s)`">
      <template #actions>
    ...
    <div class="card-premium rounded-2xl p-4 mb-6 flex flex-wrap gap-3 items-center">
      <div class="relative flex-1 min-w-[200px]">
        ...
        <input v-model="search" type="search" placeholder="N° transfert, entrepôt…" class="input-field pl-10" @input="onSearch" />
      </div>
          ...
          <div v-for="t in enCours" :key="t.id" class="card-premium rounded-2xl p-5 border-l-4"
            ...
            <div class="flex items-center gap-2 mb-3">
              ...
              <div class="flex-1 p-2 rounded-lg bg-muted/50 text-center">
                ...
                <div class="text-xs font-bold text-foreground truncate">{{ t.entrepotDestinationNom?.split(' ')[0] }}</div>
              </div>
            </div>
            <div class="text-xs text-muted-foreground mb-3">{{ t.lignes?.length ?? 0 }} produit(s) · {{ t.creePar }}</div>
            ...
            <tr v-for="t in filtered" :key="t.id" class="table-row-hover border-b border-border/50 last:border-0">
              ...
              <td class="px-4 py-3.5">
                <div class="flex items-center gap-2 text-sm">
                  ...
                  <span class="font-medium text-foreground">{{ t.entrepotDestinationNom }}</span>
                </div>
                <div class="text-xs text-muted-foreground mt-0.5">{{ t.lignes?.length ?? 0 }} produit(s)</div>
              </td>
...
const filtered = computed(() => items.value.filter(t => {
  if (search.value && !`${t.numero} ${t.entrepotSourceNom} ${t.entrepotDestinationNom}`.toLowerCase().includes(search.value.toLowerCase())) return false
  if (filterStatut.value && t.statut !== filterStatut.value) return false
  return true
}))
const enCours = computed(() => items.value.filter(t => ['BROUILLON', 'EXPEDIE'].includes(t.statut)))

async function load() {
  loading.value = true; error.value = ''
  try {
    const data = await transferService.findAll(page.value, 50)
    items.value = data.content
    total.value = data.totalElements
    totalPages.value = data.totalPages
  } catch (e: any) {
    error.value = e?.response?.data?.message || 'Erreur lors du chargement des transferts'
  } finally {
    loading.value = false
  }
}
