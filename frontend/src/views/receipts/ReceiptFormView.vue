<template>
  <div class="p-6 max-w-[1000px] mx-auto">
    <PageHeader title="Nouveau bon de réception" subtitle="Saisie des articles reçus" back="Bons de réception" />
    <form @submit.prevent="submit" class="space-y-6">
      <div class="card-premium rounded-2xl p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div><label class="label-field">Date de réception *</label><input v-model="form.dateReception" type="date" class="input-field" required /></div>
          <div><label class="label-field">Note</label><input v-model="form.note" type="text" class="input-field" placeholder="Observations…" /></div>
        </div>
      </div>
      <div class="card-premium rounded-2xl overflow-hidden">
        <div class="px-6 py-4 border-b border-border flex items-center justify-between">
          <h3 class="font-semibold">Lignes de réception</h3>
        ...
        <div class="p-4 space-y-3">
          <div v-if="!form.lignes.length" class="text-center py-10 text-sm text-muted-foreground"><Package :size="28" class="mx-auto mb-2 opacity-30" />Aucune ligne — cliquez sur "Ajouter une ligne"</div>
          <div v-for="(l, i) in form.lignes" :key="i" class="flex items-end gap-3 p-4 rounded-xl bg-muted/30 border border-border/50">
            ...
            <div class="w-28"><label class="label-field">Qté attendue</label><input v-model.number="l.qteAttendue" type="number" min="0" class="input-field" /></div>
            <div class="w-28"><label class="label-field">Qté reçue *</label><input v-model.number="l.qteRecue" type="number" min="0" class="input-field" required /></div>
            <div class="flex items-center gap-1.5 pb-2.5"><label class="flex items-center gap-1.5 text-xs cursor-pointer"><input type="checkbox" v-model="l.qualiteOk" class="accent-emerald-600" /><span class="text-muted-foreground">OK qualité</span></label></div>
      ...
      <div class="flex items-center gap-3 justify-end">
        <RouterLink to="/receipts" class="px-4 py-2.5 rounded-xl border border-border text-sm font-medium hover:bg-muted transition-colors">Annuler</RouterLink>
